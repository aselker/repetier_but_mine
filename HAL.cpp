...
ISR(PWM_TIMER_VECTOR)
{
    ...
    if(pwm_count_heater == 0)
    {
        ...
        if((pwm_pos_set[NUM_EXTRUDER] = pwm_pos[NUM_EXTRUDER])>0) WRITE(HEATED_BED_HEATER_PIN,!D8_INVERTED); /////////////
#endif
    }
    if(pwm_count==0)
    {
...
#endif
#if HEATED_BED_HEATER_PIN>-1 && HAVE_HEATED_BED
    if(pwm_pos_set[NUM_EXTRUDER] == pwm_count_heater && pwm_pos_set[NUM_EXTRUDER]!=HEATER_PWM_MASK) WRITE(HEATED_BED_HEATER_PIN,D8_INVERTED); ///////////////////////
#endif
    HAL::allowInterrupts();
